...
- **Box de fronteira** — Automação do SUAP (fora do escopo)
...
- **Apêndice IFMA** — Recursos da nossa conta (Gems, NotebookLM)
...
4. **Atenção à idade no Ensino Integrado.** Os recursos de IA voltados diretamente ao **aluno** (quizzes interativos, tutores) exigem 18+. Você pode usar a IA para *preparar* o material; o que é entregue diretamente a alunos menores de idade precisa de cuidado. Na dúvida, confirme a política da conta.
...
| Print do SUAP com dados | Não colar; transcreva só o trecho técnico já anonimizado |
...
> 🔎 **Por que isso importa:** a LGPD (Lei 13.709/2018) vale mesmo sem norma interna do IFMA. A responsabilidade por um vazamento recai sobre quem colou o dado.
...
**Prompt pronto**
> "Atue como professor do ensino técnico. Escreva uma resposta de e-mail breve, clara e cordial para um aluno que perguntou o seguinte: `[resuma a dúvida, sem nomes]`. A resposta deve `[informar / orientar / tranquilizar]` e terminar com uma frase de incentivo. Não use linguagem rebuscada."
...
**Prompt pronto**
> "Transforme os avisos abaixo em um comunicado curto e organizado para o mural de uma turma do ensino técnico. Use tom direto e amigável, destaque a data principal e termine com o que o aluno precisa fazer. Avisos: `[cole seus tópicos]`."
...
**Prompt pronto**
> "Sou professor de programação. Os alunos cometeram os erros abaixo numa atividade. Para cada erro, escreva um feedback curto, respeitoso e que ajude o aluno a entender o que rever — sem dar a resposta pronta. Erros: `[liste os erros, sem nomes]`."
...
**Prompt pronto**
> "Atue como professor de programação do ensino técnico. Esboce um plano de aula de `[duração]` sobre `[tema, ex.: estruturas de repetição em Python]` para turma de nível `[iniciante/intermediário]`. Inclua: objetivo, sequência de atividades com tempo estimado, e uma atividade prática curta. Não inclua avaliação formal."
...
**Prompt pronto**
> "Reescreva o texto abaixo para alunos iniciantes do ensino técnico, mantendo a precisão técnica mas usando linguagem simples e um exemplo do cotidiano. Não invente informação que não esteja no texto. Texto: `[cole aqui]`."
...
**Prompt pronto**
> "Crie a estrutura de uma apresentação de `[8]` slides sobre `[tema]` para alunos do ensino técnico. Para cada slide, dê um título e até 3 tópicos curtos. Comece com um slide de abertura e termine com um de resumo. Linguagem simples."
...
**Prompt pronto**
> "Atue como professor de programação do ensino técnico. Crie `[3]` questões `[discursivas / de múltipla escolha]` sobre `[conteúdo]`, nível iniciante. Para cada uma, forneça enunciado e gabarito comentado. Se houver código, limite a `[10]` linhas."
...
**Passo a passo**
1. Exporte do SUAP/Planilhas e **remova identificadores**.
...
> 🔞 **Idade:** recursos de IA voltados ao **aluno** exigem 18+. Para o professor *gerar* o quiz, não há restrição; atenção no Ensino Integrado.

**Passo a passo — Caminho A (recomendado, com Gemini ativo na conta IFMA)**
...
> "Sou professor de programação do ensino técnico. Gere `[5]` questões de múltipla escolha sobre `[conteúdo]`, nível iniciante, cada uma com 4 alternativas e indicação da correta. Se houver código, no máximo `[8]` linhas."
...
> ℹ️ Lembre-se: a IA **não acessa o SUAP**. Ela ajuda a redigir o texto; o lançamento no sistema é sempre feito por você. (Ver box de fronteira ao final.)
...
Embora seja **tecnicamente possível** automatizar tarefas no SUAP (ele é desenvolvido em Python/Django e possui API oficial), **optamos por não incluí-lo neste guia**, por três razões:

1. **Proteção de dados (LGPD):** o SUAP contém dados pessoais de alunos; seu tratamento por IA externa exige base legal e controles institucionais que estão fora do alcance do docente individual — e a LGPD incide independentemente de norma interna.
2. **Ausência de regulamentação:** o IFMA ainda não possui norma sobre uso de IA articulado ao SUAP. Na falta dela, o caminho responsável é a prudência, não a iniciativa individual.
3. **Escopo:** este guia trata das tarefas já assistidas pela IA institucional (Workspace/Gemini); o SUAP não tem essa integração.

*O caminho existe e é legítimo para quem quiser aprofundar com a devida autorização — mas ensiná-lo aqui fugiria ao propósito deste material.*
...
**Prompt:** "Você é um professor de ensino técnico, atencioso e objetivo. Dúvida do aluno (anonimizada): `[...]`. Tarefa: resposta de e-mail. Tom: cordial e encorajador. Restrições: máx. 6 linhas; não invente prazos — use `[CONFIRMAR]` onde faltar dado. Formato: assunto + corpo."
...
**Prompt pronto**
> "Crie um infográfico vertical explicativo sobre `[tema]`, para alunos do ensino técnico. Use ícones simples, setas de fluxo e blocos de texto curtos. Paleta de cores `[ex.: azul e cinza]`. Mantenha o texto correto e legível."
...
---
# APÊNDICE IFMA — RECURSOS ESPECÍFICOS DA NOSSA CONTA
*Camada institucional. O núcleo do guia (Seções 0–5) funciona em qualquer escola; este apêndice é o que aproveita o que temos no IFMA. Mantido separado de propósito, para que a futura versão genérica nasça por extração do núcleo.*

> **Pré-requisito comum:** todos os recursos abaixo dependem de estar logado na conta `@ifma.edu.br`, onde o Gemini já está integrado. Isso garante a proteção de dados de nível institucional (dados não usados para treinar modelos fora do domínio). Mesmo assim, a **Régua de Segurança (Seção 1) continua valendo** — proteção de dados não dispensa anonimização.

---

## IFMA.1 — Gems: tutores personalizados por disciplina
...
2. Defina o papel: "tutor da disciplina X, nível ensino técnico".
...
> 🔞 **Idade:** os recursos de IA do Classroom voltados ao **aluno** exigem 18+. Em turmas do Integrado (com menores), use o Gem como ferramenta sua, não como recurso entregue diretamente aos alunos menores de idade. Verifique a política antes.
...
## IFMA.2 — NotebookLM: caderno de estudos a partir dos seus materiais
...
> **Nota para a versão genérica (v2):** ao extrair o guia para qualquer escola, este Apêndice IFMA sai inteiro e é substituído pela realidade da outra instituição (sistema acadêmico próprio, plataformas adotadas). O núcleo (Seções 0–5) permanece igual.
